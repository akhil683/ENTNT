...
export function CandidatesKanbanDraggable({
  ...
  const [draggedCandidates, setDraggedCandidates] =
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    const updatedCandidates = draggedCandidates.map((c) =>
      c.id === candidateId
        ? { ...c, stage: newStage, updatedAt: new Date() }
        : c,
    );
    ...
    try {
      ...
      const updatedCandidate = await mockApi.updateCandidate(candidateId, {
        stage: newStage,
      });
  ...
  const candidatesByStage = stages.reduce(
    (acc, stage) => {
      acc[stage.value] = draggedCandidates.filter(
